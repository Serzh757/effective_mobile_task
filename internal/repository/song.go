...
func (r *SongRepository) CreateSong(req *models.Song) (*models.Song, error) {
	if _, err := r.DB.Model(req).Insert(); err != nil {
		log.Info().Err(err).Msg("Failed to add new song")
		return nil, err
	}

	lastInsertSong := &models.Song{}
	err := r.DB.Model(lastInsertSong).Where("song.id = ?", req.ID).Select()
	if err != nil {
		log.Info().Err(err).Msg("Failed to get new song")
		return nil, err
	}
...
func (r *SongRepository) GetAllSongs(page, limit int) ([]models.Song, error) {
	...
	if err != nil {
		log.Info().Err(err).Msg("Failed to get all songs")
		return nil, err
	}
	return songs, nil
}
...
func (r *SongRepository) GetSongByID(songID uint) (*models.Song, error) {
	...
	err := r.DB.Model(&song).Where("song.id = ?", songID).Select()
	if err != nil {
		log.Info().Err(err).Msg("Failed to retrieve song with ID")
		return nil, err
	}
	return &song, nil
}
...
func (r *SongRepository) UpdateSong(req *models.Song) (*models.Song, error) {
	...
	if err != nil {
		log.Info().Err(err).Msg("Failed to update song data")
		return nil, err
	}

	var lastUpdateSong models.Song
	err = r.DB.Model(&lastUpdateSong).Where("song.id = ?", req.ID).Select()
	if err != nil {
		log.Info().Err(err).Msg("Failed to get updated song")
		return nil, err
	}
...
func (r *SongRepository) DeleteSong(songID uint) error {
	...
	if err := r.DB.Model(song).Where("song.id = ?", song.ID).Select(); err != nil {
		log.Info().Err(err).Msg("Failed to get song")
		return err
	}
